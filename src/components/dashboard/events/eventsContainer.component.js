  ...
  if (!orgs || orgs.length === 0) {
    return (
      <div className="emptyDBContainer">
        ...
        <Link to="signupOrg"><button className="standardButtonWithoutColour mcGreyBG"> <FontAwesomeIcon icon={faPlus} style={{marginRight: "0.3em"}} /> Create Organisation </button></Link>
      </div>
    )
  }

  if (selectedOrg) {
    return <EventAccordionContainer org={selectedOrg} setSelectedOrg={setSelectedOrg} />
  } else {
    return (
      <>
        ...
        <div className="cardsDashContainer">
          {
            orgs.map((org) =>{
              return (
                <div key={org._id} className="eventCardDash grow" onClick={() => setSelectedOrg(org)}>
                  <StandardCard name={org.name} image={org.imageURL} setSelectedOrg={setSelectedOrg} />
                </div>
              )
            })
          }
