...
import { acceptVolunteerRequest, declineVolunteerRequest } from '../../../../Actions/projectActions'
  ...
  return (
    <div className="projectDashCard">
      <div className="headerButtonsContainer">
        <button className="standardButtonWithoutColour mcGreenBG" onClick={() => setSelectedRequest(null)}> <FontAwesomeIcon icon={faChevronCircleLeft} style={{textAlign: 'left', marginRight: "0.3em"}}/> Back </button>
      </div>
      <div className="volunteerRequestSelected">
        ...
        <div className="volunteerRequestSelectedInfo">
          ...
          <p className="projectText"> {volunteer.firstName} {volunteer.lastName} </p>
          ...
          <p className="projectText"> {volunteer.city}, {volunteer.country} </p>
        </div>
...
const VolunteerRequestButtons = ({ request }) => {
  ...
  if (request.status === "PENDING") {
    return (
      <>
        <button onClick={() => dispatch(acceptVolunteerRequest(request._id))} className="standardButtonWithoutColour mcGreenBG" style={{height: 34, width: 34, marginRight:'8px'}}> <FontAwesomeIcon icon={faCheck} /> </button>
        <button onClick={() => dispatch(declineVolunteerRequest(request._id))} className="standardButtonWithoutColour mcRedBG" style={{height: 34, width: 34}}> <FontAwesomeIcon icon={faTimes} /> </button>
      </>
    )
    ...
    <div className="projectDashCard projectVolunteerRequestListItem" onClick={() => setSelectedRequest(() => request._id)}>
      ...
      <div className="volunteerRequestHeader">
        ...
        <div>
          ...
          <span className="volunteerGender"> {getGenderIcon(volunteer.gender)} </span>
        </div>
        <h6> {volunteer.city}, {volunteer.country} </h6>
      </div>
    </div>
  )
}
...
const VolunteerRequirmentInfo = ({ volunteer }) => {
  return (
    <div className="projectDashCard" style={{height: 'fit-content', position: 'relative'}}>
      ...
      {/* FIX */}
      <button className="standardButtonWithoutColour mcGreenBG" onClick={() => console.log('edit')} style={{position: 'absolute', top: '8px', right: '8px'}}> <FontAwesomeIcon icon={faEdit} /> </button>
    </div>
  )
}
